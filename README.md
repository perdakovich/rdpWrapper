...
And yes, it can auto-generate offsets for new/updated Windows versions - thanks to the projects from [llccd](https://github.com/llccd):
...
> [!IMPORTANT]
> If Defender or another antivirus detects any part of RdpWrapper as malware, it may prevent proper work or cause the application to fail to start.
> If a required file exists but its loading is blocked, RdpWrapper will not start.
...
In this fork **geographic restrictions are removed**, so the
application starts and runs on any system regardless of the user's region, locale
or timezone.  
Vendor dependencies are pinned by version and are updated only after a
manual audit of upstream changes. This also removes the risk of the project
turning into protestware (sabotage, data deletion, and other unwelcome surprises)
driven by the author's political motivations.
